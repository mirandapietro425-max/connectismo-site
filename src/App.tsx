...
function Header({ active }: { active: PageKey }) {
  ...
  return <header className="site-header">
    ...
    <div className="header-inner">
       <Link href="/" className="brand" data-testid="link-brand" aria-label="ConnecTismo, página inicial"><img className="brand-logo" src="/assets/connectismo-icon-pessoas-64.png" alt="Logo ConnecTismo com três cabeças estilizadas" width="64" height="64" decoding="async" /><span className="brand-name">ConnecTismo</span></Link>
     ...
     <nav className={`mobile-nav ${open ? "open" : ""}`} aria-label="Navegação móvel">{pages.map((page) => <Link key={page.key} href={page.href} className={`nav-link ${page.key === "sobre" ? "nav-project" : ""}`} data-testid={`link-mobile-${page.key}`} aria-current={active === page.key ? "page" : undefined}>{page.label}</Link>)}</nav>
  </header>;
}
...
function PageHero({ eyebrow, title, text, image, imageAlt, links = [] }: { eyebrow: string; title: string; text: string; image?: string; imageAlt?: string; links?: { href: string; label: string }[] }) {
  return <section className="page-hero"><div className="container reveal"><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{text}</p>{links.length > 0 && <nav className="section-nav" aria-label="Nesta página">{links.map((link) => <a href={link.href} key={link.href} data-testid={`link-section-${link.href.slice(1)}`}>{link.label}</a>)}</nav>}</div>{image && <div className="page-hero-image"><img src={image} alt={imageAlt ?? ""} loading="lazy" decoding="async" /></div>}</section>;
}
...
function VideoCard({ video, className = "" }: { video: LocalVideo; className?: string }) {
  return <article className={`video-card ${className}`} data-testid={`video-card-${video.context.toLowerCase().replaceAll(" ", "-")}`}>
    ...
    <div className="video-frame"><video controls preload="metadata" poster={video.poster} playsInline aria-label={`${video.title}. Duração aproximada: ${video.duration}`}><source src={video.src} type="video/mp4" />Seu navegador não consegue reproduzir este vídeo local.</video></div>
...
function HomePage() {
  return <>
    ...
    <section className="signs-section" id="sinais"><div className="container signs-grid"><div><SectionKicker>Observar sem rotular</SectionKicker><h2>Sinais pedem escuta, não conclusões rápidas.</h2><p className="lede">Diferenças na comunicação, na interação, no processamento sensorial ou na necessidade de previsibilidade podem aparecer de muitos modos. Uma lista não diagnostica ninguém.</p><Link href="/sobre" className="button button-dark" data-testid="link-signs-about">Entender o projeto <ArrowRight size={16} /></Link></div><div className="signs-list"><div><span className="number">01</span><p>Formas próprias de comunicar, brincar, aprender ou demonstrar interesse.</p></div><div><span className="number">02</span><p>Maior ou menor sensibilidade a sons, luzes, cheiros, texturas e movimento.</p></div><div><span className="number">03</span><p>Necessidade de previsibilidade, repetição ou tempo diferente para transições.</p></div><small>Se houver preocupação com desenvolvimento ou bem-estar, procure avaliação e orientação profissional.</small></div></div></section>
        <section className="home-band" id="suporte"><div className="container support-intro"><div className="support-copy"><SectionKicker>Um espectro, muitas experiências</SectionKicker><h2>Suporte não mede valor.</h2><p className="lede">Os níveis de suporte indicam quanto apoio uma pessoa pode precisar em diferentes contextos. Não são rótulos fixos e não medem inteligência, potencial ou importância.</p></div><figure className="support-visual"><img src="/assets/editorial/official/inicio/suporte_contexto.png" alt="Pessoa usando cartões de comunicação com apoio de uma profissional, enquanto uma família acolhe uma criança com fones abafadores" loading="lazy" decoding="async" /><figcaption>Uma referência visual para lembrar que apoio é contexto, não medida de valor.</figcaption></figure></div><div className="container support-levels" aria-label="Descrição dos níveis de suporte no TEA"><article className="support-level support-level-1"><div className="support-level-top"><strong>01</strong><span>Requer suporte</span></div><p className="support-old-term">Termo antigo: “autismo leve”</p><dl><div><dt>Autonomia</dt><dd>Independente nas tarefas básicas do dia a dia.</dd></div><div><dt>Comunicação e socialização</dt><dd>Fala bem, mas pode ter dificuldades com regras sociais sutis, piadas ou para iniciar conversas.</dd></div><div><dt>Desafios</dt><dd>Imprevistos, mudanças na rotina e sobrecarga sensorial podem exigir muita energia de adaptação.</dd></div></dl></article><article className="support-level support-level-2"><div className="support-level-top"><strong>02</strong><span>Requer suporte substancial</span></div><p className="support-old-term">Termo antigo: “autismo moderado”</p><dl><div><dt>Autonomia</dt><dd>Precisa de supervisão e apoio frequente em várias atividades cotidianas.</dd></div><div><dt>Comunicação e socialização</dt><dd>Pode apresentar limitações evidentes na fala, linguagem muito literal ou dificuldade em manter interações.</dd></div><div><dt>Desafios</dt><dd>Mudanças, rigidez de rotina e imprevistos podem provocar crises mais visíveis.</dd></div></dl></article><article className="support-level support-level-3"><div className="support-level-top"><strong>03</strong><span>Requer suporte muito substancial</span></div><p className="support-old-term">Termo antigo: “autismo severo”</p><dl><div><dt>Autonomia</dt><dd>A dependência de apoio pode ser constante para quase todas as atividades da vida.</dd></div><div><dt>Comunicação e socialização</dt><dd>A comunicação verbal pode ser muito limitada ou ausente, exigindo recursos visuais ou comunicação alternativa.</dd></div><div><dt>Desafios</dt><dd>Rigidez intensa, comportamentos repetitivos e dificuldade de adaptação podem exigir suporte contínuo.</dd></div></dl></article></div><div className="container support-disclaimer"><strong>Importante:</strong> os níveis não são uma escala de funcionamento e não descrevem tudo o que uma pessoa consegue fazer. A avaliação deve considerar a pessoa, o contexto e os apoios necessários.</div></section>
...
function InclusionPage() {
  return <><PageHero eyebrow="Inclusão" title="Não basta estar presente." text="Inclusão é ter acesso, voz, escolhas, apoio e o direito de participar sem precisar esconder quem se é." image="/assets/editorial/novas/inclusao/connectismo_inclusao.png" links={[{ href: "#contextos", label: "Contextos" }, { href: "#direitos", label: "Direitos" }, { href: "#atitudes", label: "Atitudes" }]} /><section className="page-body"><div className="container page-layout"><aside className="page-sticky"><SectionKicker>Participação</SectionKicker><h2>O mundo também pode se adaptar.</h2><p className="lede">A escola, o trabalho e os espaços sociais ficam melhores quando diferentes formas de estar são previstas desde o início.</p><div className="reading-progress"><span>Você está em</span><strong>Inclusão</strong><small>1 de 5 temas</small></div></aside><div className="page-copy"><VideoCard video={localVideos.inclusion} className="inclusion-main-video" /><div className="content-heading" id="contextos"><SectionKicker>Onde acontece</SectionKicker><h2>O contexto muda. O direito de participar, não.</h2></div><div className="image-topic-grid"><EditorialImage src="/assets/editorial/official/inclusao/inclusao_escolar.png" alt="Pessoa aprendendo em um ambiente escolar colaborativo" label="Escola: diferentes formas de demonstrar o que se aprendeu." /><EditorialImage src="/assets/editorial/official/inclusao/inclusao_trabalho.png" alt="Pessoas colaborando em um ambiente profissional" label="Trabalho: combinar expectativas e reduzir barreiras." /><EditorialImage src="/assets/editorial/official/inclusao/inclusao_social.png" alt="Pessoas convivendo em uma situação social" label="Vida social: escutar preferências e respeitar pausas." /></div><div className="support-grid"><article className="support-item"><span className="number">01</span><h3>Inclusão escolar</h3><p>Planejar acessibilidade, comunicação clara e diferentes formas de demonstrar o que se aprendeu.</p></article><article className="support-item"><span className="number">02</span><h3>Mercado de trabalho</h3><p>Combinar expectativas, reduzir barreiras sensoriais e reconhecer talentos sem exigir máscaras sociais.</p></article><article className="support-item"><span className="number">03</span><h3>Vida social e familiar</h3><p>Escutar preferências, respeitar pausas e construir redes de apoio que não deixem ninguém sozinho.</p></article></div><div id="direitos" className="content-heading"><SectionKicker>Direitos</SectionKicker><h2>Informação fortalece escolhas.</h2></div><p>O Estatuto da Pessoa com Deficiência e a Lei nº 12.764/2012 asseguram direitos fundamentais às pessoas autistas, incluindo educação, saúde, trabalho, acessibilidade e proteção contra discriminação.</p><section className="cordoes-block" aria-labelledby="cordoes-title"><div className="cordoes-heading"><SectionKicker>Identificação e respeito</SectionKicker><h3 id="cordoes-title">Cordões podem comunicar uma necessidade, não uma obrigação.</h3><p>Algumas pessoas usam cordões para sinalizar uma deficiência oculta ou uma identidade neurodivergente. O uso é voluntário, os significados podem variar e o cordão nunca substitui uma conversa respeitosa.</p></div><div className="cordoes-grid"><figure className="cordoes-visual"><img src="/assets/editorial/official/inclusao/cordoes_tea_novos.png" alt="Três cordões de identificação: um com girassóis, um com o símbolo do infinito colorido e um com peças de quebra-cabeça" loading="lazy" decoding="async" /><figcaption>Referência visual enviada para o projeto. Símbolos não são universais: pergunte como a pessoa prefere ser identificada.</figcaption></figure><div className="cordoes-meanings"><article><span className="number">01</span><h4>Cordão de girassol</h4><p>É associado a deficiências ocultas e pode indicar que a pessoa precisa de mais tempo, comunicação clara, prioridade ou ajuda.</p></article><article><span className="number">02</span><h4>Cordão do infinito colorido</h4><p>É usado por algumas pessoas para representar a neurodiversidade, incluindo TEA, TDAH e dislexia. A escolha do símbolo é individual.</p></article><article><span className="number">03</span><h4>Cordão quebra-cabeça</h4><p>Também aparece em materiais sobre autismo, mas é um símbolo controverso para parte da comunidade. Respeite a preferência da pessoa.</p></article></div></div><div className="law-panel"><div><span className="eyebrow">O que a lei garante</span><h4>O cordão não é documento oficial.</h4><p>Direitos não dependem de usar um cordão. No Brasil, a proteção legal vem do diagnóstico e da legislação aplicável, com avaliação individual das necessidades.</p></div><ul className="law-list"><li><a href="https://www.planalto.gov.br/ccivil_03/_ato2011-2014/2012/lei/l12764.htm" target="_blank" rel="noreferrer">Lei nº 12.764/2012</a><span>Institui a Política Nacional de Proteção dos Direitos da Pessoa com TEA e reconhece a pessoa autista como pessoa com deficiência para todos os efeitos legais.</span></li><li><a href="https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2015/lei/l13146.htm" target="_blank" rel="noreferrer">Lei nº 13.146/2015</a><span>Estabelece o Estatuto da Pessoa com Deficiência e direitos de acessibilidade, participação e não discriminação.</span></li><li><a href="https://www.planalto.gov.br/ccivil_03/_ato2019-2022/2020/lei/l13977.htm" target="_blank" rel="noreferrer">Lei nº 13.977/2020</a><span>Cria a CIPTEA, carteira de identificação que facilita o reconhecimento e o atendimento prioritário quando necessário.</span></li></ul></div></section><div className="callout"><strong>CIPTEA</strong><span>A Carteira de Identificação da Pessoa com TEA facilita o acesso a prioridades e serviços. Conhecer a legislação fortalece a autonomia para reivindicar direitos.</span></div><div id="atitudes" className="content-heading"><SectionKicker>Agora</SectionKicker><h2>Três atitudes possíveis hoje.</h2></div><ol className="number-list"><li><span className="number">01</span><span><strong>Pergunte, não presuma.</strong><br />A pessoa é a melhor fonte sobre suas necessidades.</span></li><li><span className="number">02</span><span><strong>Ofereça alternativas.</strong><br />Mais de um caminho pode levar à participação.</span></li><li><span className="number">03</span><span><strong>Repare as barreiras.</strong><br />O ambiente também precisa mudar.</span></li></ol><NextPath href="/dicas" label="Próximo tema" title="Apoio no cotidiano" /></div></div></section></>;
}
...
function QuizPage() {
  ...
  return <><PageHero eyebrow="Mitos & verdades" title="Ciência contra a desinformação." text="A informação precisa ajuda a combater preconceitos e construir uma participação mais justa." image="/assets/editorial/novas/mitos-verdades/connectismo_mitos.png" links={[{ href: "#contexto", label: "Contexto" }, { href: "#quiz", label: "Quiz TEA" }]} /><section className="page-body"><div className="container page-layout"><aside className="page-sticky"><SectionKicker>Teste seus conhecimentos</SectionKicker><h2>Informação também se pratica.</h2><p className="lede">Não é prova: é um convite para rever ideias e continuar aprendendo.</p><div className="reading-progress"><span>Você está em</span><strong>Mitos & verdades</strong><small>4 de 5 temas</small></div></aside><div className="page-copy"><div id="contexto" className="myths-video-grid"><VideoCard video={localVideos.myths} /><VideoCard video={localVideos.learning} /></div><div className="myths-context"><img src="/assets/editorial/variadas/mitos-verdades/variadas_mitos_laboratorio.png" alt="Mesa com materiais de pesquisa e anotações" loading="lazy" decoding="async" /><div><SectionKicker>Antes do quiz</SectionKicker><h2>Uma boa pergunta checa a fonte.</h2><p>Procure informação que reconheça a diversidade do espectro e evite promessas fáceis. Experiência pessoal, pesquisa e orientação profissional podem ocupar lugares diferentes. Nenhum deles deve ser usado para apagar a pessoa.</p></div></div><div id="quiz" className="quiz-shell"><div><SectionKicker>Quiz TEA</SectionKicker><h2>Repare nas ideias que chegam até você.</h2><p className="lede">Escolha uma alternativa para cada questão. O feedback aparece depois do envio.</p></div><div><div className="quiz-result" aria-live="polite"><strong>{submitted ? `Você acertou ${score} de ${questions.length}.` : "Antes de responder"}</strong><p>{submitted ? (score >= 3 ? "Ótimo ponto de partida. Continue ouvindo as experiências da comunidade." : "Toda resposta é uma oportunidade de aprender.") : "Responda todas as questões para ver o resultado."}</p></div>{questions.map((question, index) => <fieldset className={`question ${submitted ? answers[index] === question.answer ? "correct" : "incorrect" : ""}`} key={question.text}><legend>{index + 1}. {question.text}</legend><div className="options">{question.options.map((option, optionIndex) => <label className="quiz-option" key={option}><input type="radio" name={`question-${index}`} checked={answers[index] === optionIndex} onChange={() => { setSubmitted(false); setAnswers((current) => ({ ...current, [index]: optionIndex })); }} />{option}</label>)}</div>{submitted && <div className="quiz-feedback"><strong>{answers[index] === question.answer ? "Resposta correta. " : "Vale revisar. "}</strong>{question.feedback}</div>}</fieldset>)}<button className="button button-primary" onClick={() => { if (Object.keys(answers).length === questions.length) setSubmitted(true); }} disabled={Object.keys(answers).length < questions.length} data-testid="button-submit-quiz"><Check size={17} /> Ver meu resultado</button></div></div><NextPath href="/sobre" label="Último tema" title="Conheça os critérios do projeto" /></div></div></section></>;
}

function AboutPage() {
  return <><PageHero eyebrow="Sobre o projeto" title="Feito com pesquisa, guiado por escuta." text="O ConnecTismo foi desenvolvido por Emilly Ravanello como Trabalho de Conclusão de Curso, com o propósito de aproximar informações confiáveis de pessoas que vivenciam o TEA em suas diversas dimensões." image="/assets/editorial/official/sobre/sobre_bastidores.png" links={[{ href: "#metodo", label: "Método" }, { href: "#navegar", label: "Como navegar" }]} /><section className="page-body"><div className="container page-layout"><aside className="page-sticky"><SectionKicker>Por que existe</SectionKicker><h2>Informação também é acessibilidade.</h2><p className="lede">Este portal nasceu da convivência, da pesquisa e do desejo de transformar uma experiência familiar em conhecimento, conscientização e inclusão.</p><div className="reading-progress"><span>Você está em</span><strong>O projeto</strong><small>5 de 5 temas</small></div></aside><div className="page-copy"><div className="about-note"><h2>Informação que acolhe e transforma.</h2><p>Meu nome é Emilly Ravanello e sou estudante da Escola Prof. Maria Rocha, onde faço o Ensino Médio Integrado ao Curso Técnico em Informática. Durante minha formação, tive a oportunidade de aprender não apenas conteúdos relacionados à informática, mas também a desenvolver projetos que podem contribuir de alguma forma para a sociedade.</p><p>A escolha do tema do meu TCC, relacionado ao Transtorno do Espectro Autista, surgiu principalmente por uma razão muito especial para mim: meu sobrinho, que tem 5 anos e está dentro do espectro autista. A convivência com ele despertou em mim uma maior curiosidade e interesse em conhecer melhor o autismo, suas características, os diferentes níveis de suporte e, principalmente, a importância da inclusão e da informação.</p><p>Por isso, decidi unir esse interesse pessoal com os conhecimentos que adquiri no Curso Técnico em Informática. Meu objetivo é desenvolver um projeto que possa levar informação de forma simples, acessível e acolhedora, ajudando a combater preconceitos e a promover uma maior compreensão sobre o autismo.</p><p>Assim, este TCC representa não apenas uma atividade para concluir minha formação, mas também uma forma de transformar uma experiência da minha família em conhecimento e conscientização, utilizando a tecnologia como ferramenta para contribuir com a inclusão.</p></div><div id="metodo" className="about-values"><article className="value-card"><span className="eyebrow">01 / propósito</span><h3>Empatia</h3><p>Começar pelo lugar e pela experiência da pessoa.</p></article><article className="value-card"><span className="eyebrow">02 / pesquisa</span><h3>Precisão</h3><p>Buscar fontes científicas, legislação e contexto.</p></article><article className="value-card"><span className="eyebrow">03 / compromisso</span><h3>Inclusão</h3><p>Transformar presença em participação real.</p></article></div><div id="navegar" className="content-heading"><SectionKicker>Como navegar por aqui</SectionKicker><h2>Volte quando o momento mudar.</h2></div><p>Comece pelo tema que mais se aproxima do seu momento. Consulte as dicas quando precisar de uma estratégia concreta. Visite cultura para ampliar repertórios e use o quiz para desmontar certezas rápidas.</p><div className="callout"><strong>Um compromisso simples</strong><span>Levar informação de forma simples, acessível e acolhedora, combatendo preconceitos sem transformar a diferença em defeito.</span></div><div className="about-route-grid"><Link href="/inclusao" data-testid="link-about-inclusion"><GraduationCap size={18} /> Inclusão e direitos <ArrowRight size={15} /></Link><Link href="/dicas" data-testid="link-about-tips"><Compass size={18} /> Apoio cotidiano <ArrowRight size={15} /></Link><Link href="/cultura" data-testid="link-about-culture"><BookOpen size={18} /> Repertório cultural <ArrowRight size={15} /></Link></div></div></div></section></>;
}
...
function Footer() {
  return <footer className="footer"><div className="container"><div className="footer-grid"><div><Link href="/" className="brand" data-testid="link-footer-brand" aria-label="ConnecTismo, página inicial"><img className="brand-logo" src="/assets/connectismo-icon-pessoas-64.png" alt="Logo ConnecTismo com três cabeças estilizadas" width="64" height="64" decoding="async" /><span className="brand-name">ConnecTismo</span></Link><p style={{ marginTop: "1.2rem" }}>Um portal português sobre autismo, inclusão, apoio cotidiano e representação.</p></div><div><h3>Explorar</h3><ul>{pages.slice(1, 5).map((page) => <li key={page.key}><Link href={page.href} data-testid={`link-footer-${page.key}`}>{page.label}</Link></li>)}</ul></div><div><h3>Projeto</h3><ul><li><Link href="/sobre" data-testid="link-footer-about">Sobre o projeto</Link></li><li><a href="#conteudo" data-testid="link-footer-top">Voltar ao início</a></li></ul></div></div><div className="footer-bottom"><span>© 2026 ConnecTismo. Conteúdo educativo.</span><button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} data-testid="button-scroll-top">Voltar ao início <ArrowDown size={13} style={{ transform: "rotate(180deg)" }} /></button></div></div></footer>;
}
